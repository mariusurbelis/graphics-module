...
void display()
{
	...
	vec3 lightPosition = vec3(1.5, 1.0, 0.8);

	// Camera matrix
	mat4 view = lookAt(
		vec3(0, 0, 20), // Camera is at (0,0,4), in World Space
	...
	// This block of code draws the sphere that represents the light
	model.push(model.top());
	{
		model.top() = translate(model.top(), lightPosition);
		model.top() = scale(model.top(), vec3(model_scale / 6.f, model_scale / 6.f, model_scale / 6.f));//scale equally in all axis
